...
public class VTSTools {
	...
	private static final double STEP_OEM = 10;
	...
	public static void generateLegSequenceMEMFile(final String path,
		...
		try {
			...
			for (final AttitudeLeg leg : sequence) {
				...
				// Building the line for the start of the leg
				bf = new StringBuffer();
				bf.append(days);
				bf.append(SEP);
				bf.append(seconds);
				...
				// Building the line for the end of the leg
				bf = new StringBuffer();
				bf.append(days);
				bf.append(SEP);
				bf.append(seconds);
	 ...
	 * [DO NOT MODIFY THIS METHOD]
	 * 
	 * Writes a CIC MEM file to visualize the input {@link Timeline} in VTS.<br>
	 * 
	 * @param path     Path of the output file
	 * @param sequence Input {@link StrictAttitudeLegsSequence} to be written
	 */
	public static void generateTimelineMEMFile(final String path, final Timeline timeline) {
		...
		try {
			...
			fw.append("META_START");
			fw.append(NL);
			fw.append("COMMENT SATELLITE EVENTS");
			fw.append(NL);
			fw.append("OBJECT_NAME = PATRIUS SATELLITE");
			fw.append(NL);
			fw.append("OBJECT_ID = PAT001");
			...
			for (final Phenomenon phen : timeline.getPhenomenaList()) {
				...
				// Getting the start date
				final AbsoluteDate start = phen.getStartingEvent().getDate();
				seconds = start.offsetFrom(REFERENCE_DATE, utc) + utc.offsetFromTAI(start);
				days = (int) (seconds / Constants.JULIAN_DAY);
				seconds = seconds - days * Constants.JULIAN_DAY;

				// Building the line for the start event of the Phenomenon
				bf = new StringBuffer();
				bf.append(days);
				bf.append(SEP);
				bf.append(seconds);
				bf.append(SEP);
				bf.append("\"Start of " + phen.getCode() + "\"");
				bf.append(SEP);
				fw.append(bf.toString());
				fw.append(NL);

				// Getting the end date
				final AbsoluteDate end = phen.getEndingEvent().getDate();
				seconds = end.offsetFrom(REFERENCE_DATE, utc) + utc.offsetFromTAI(end);
				days = (int) (seconds / Constants.JULIAN_DAY);
				seconds = seconds - days * Constants.JULIAN_DAY;
